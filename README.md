...
The app can connect to the companion Supabase project from server routes. Copy
`.env.example` to `.env.local` for local development and set the same two keys
in the hosted runtime:

- `SUPABASE_URL`
- `SUPABASE_ANON_KEY`

`GET /api/supabase` performs a read-only connection check against the existing
`core_dogs` table. The anon key remains server-side and is never returned by
the endpoint. Operational CRUD continues to use the existing D1 database until
