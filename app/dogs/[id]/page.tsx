...
type Puppy = BaseRecord & { litter_id: number; name: string; status: string };
...
type DataSet = { dogs: Dog[]; dog_medical_records: MedicalRecord[]; dog_registrations: Registration[]; dog_documents: DogDocument[]; transactions: Transaction[]; litters: Litter[]; puppies: Puppy[] };
...
export default function DogProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const profile = useMemo(() => {
    ...
    const costs = data.transactions.filter((item) => item.dog_id === dog.id && item.type === "Cost").sort((left, right) => String(right.paid_date ?? right.created_at).localeCompare(String(left.paid_date ?? left.created_at)));
    const litters = data.litters.filter((item) => item.dam_id === dog.id || item.sire_id === dog.id);
    const puppyCount = data.puppies.filter((puppy) => litters.some((litter) => litter.id === puppy.litter_id)).length;
    ...
    return { medical, registrations, documents, costs, litters, puppyCount, medicalCost, otherCost, totalCost: (dog.purchase_price_cents ?? 0) + medicalCost + otherCost };
  }, [data, dog]);
  ...
  return <main className="dog-profile-page">
    ...
    <div className="profile-content">
      ...
      <section className="profile-stat-grid" aria-label="Profile totals">
        <article><span>Registries</span><b>{profile.registrations.length}</b><small>Connected records</small></article>
        <article><span>Medical</span><b>{profile.medical.length}</b><small>{money(profile.medicalCost)} recorded</small></article>
        <article><span>Other costs</span><b>{money(profile.otherCost)}</b><small>{profile.costs.length} entries</small></article>
        <article><span>Lifetime total</span><b>{money(profile.totalCost)}</b><small>Purchase, medical, and costs</small></article>
        <article><span>Litters</span><b>{profile.litters.length}</b><small>{profile.puppyCount} puppies recorded</small></article>
      ...
      <div className="profile-layout">
        ...
        <section className="profile-section">
          ...
          {profile.litters.length ? <div className="profile-record-list">{profile.litters.map((litter) => <article key={litter.id}><div className="record-icon litter"><CalendarDays size={18} /></div><div><b>{litter.name}</b><p>{litter.dam_id === dog.id ? "Dam" : "Sire"} / {litter.status}</p><small>{shortDate(litter.birth_date || litter.due_date)} / {data?.puppies.filter((puppy) => puppy.litter_id === litter.id).length ?? 0} puppies</small></div></article>)}</div> : <EmptyProfileSection title="No breeding history" text="Litters connected to this dog will appear here with dates, role, status, and offspring totals." />}
        </section>
