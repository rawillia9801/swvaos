  ...
  return <main className="puppy-portal">
    ...
    <div className="portal-content">
      ...
      <div className="portal-grid">
        ...
        <section className="portal-section portal-payments"><header><span>ACCOUNT</span><h2>Payment record</h2></header><div className="portal-balance"><span><small>Purchase total</small><b>{money(data.payments.saleTotalCents)}</b></span><span><small>Payments recorded</small><b>{money(data.payments.paidCents)}</b></span><span><small>Outstanding</small><b>{money(data.payments.outstandingCents)}</b></span></div>{data.payments.items.length > 0 ? <div className="portal-payment-list">{data.payments.items.slice(0, 8).map((payment) => <span key={payment.id}><i className={["Paid", "Complete"].includes(payment.status) ? "paid" : ""}>{payment.status}</i><b>{payment.description}</b><small>{[payment.category, payment.method, shortDate(payment.paidDate || payment.dueDate)].filter(Boolean).join(" / ")}</small><strong>{money(payment.amountCents)}</strong></span>)}</div> : <div className="portal-empty compact">No payments have been recorded yet.</div>}</section>
