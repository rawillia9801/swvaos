...
import { ArrowRight, CalendarDays, CheckCircle2, CircleDollarSign, Dog, Download, FileSignature, HeartPulse, ShieldCheck } from "lucide-react";

type Puppy = { id: number; name: string; sex: string; color: string; birthDate: string; currentWeight: number; status: string; priceCents: number; notes: string };
...
type Payment = { id: number; description: string; amountCents: number; status: string; dueDate: string; paidDate: string };
type PortalData = { buyer: { id: number; name: string; email: string; phone: string; location: string }; puppies: Puppy[]; updates: Update[]; contracts: Contract[]; payments: { paidCents: number; outstandingCents: number; items: Payment[] } };
    ...
    <div className="portal-content">
      ...
      <div className="portal-grid">
        <section className="portal-section portal-puppies"><header><span>MY PUPPY</span><h2>Assigned puppy records</h2></header>{data.puppies.length ? data.puppies.map((puppy) => {
          ...
          return <article key={puppy.id} className="portal-puppy"><div className="portal-puppy-head"><span className="portal-avatar">{puppy.name.slice(0, 1).toUpperCase()}</span><div><h3>{puppy.name}</h3><p>{[puppy.sex, puppy.color, puppy.status].filter(Boolean).join(" / ")}</p></div><i>{puppy.status}</i></div><div className="portal-puppy-facts"><span><small>Birthday</small><b>{shortDate(puppy.birthDate)}</b></span><span><small>Latest weight</small><b>{puppy.currentWeight ? `${puppy.currentWeight} lb` : "Not recorded"}</b></span><span><small>Recorded price</small><b>{money(puppy.priceCents)}</b></span></div>{updates.length > 0 && <div className="portal-update-list">{updates.slice(0, 5).map((update) => <div key={update.id}><span><CalendarDays size={15} /><b>{update.title}</b><small>{shortDate(update.createdAt)}</small></span><p>{update.body}</p>{(update.weekNumber || update.weight) && <i>{[update.weekNumber ? `Week ${update.weekNumber}` : "", update.weight ? `${update.weight} lb` : ""].filter(Boolean).join(" / ")}</i>}</div>)}</div>}</article>;
        ...
        <section className="portal-section portal-payments"><header><span>ACCOUNT</span><h2>Payment record</h2></header><div className="portal-balance"><span><small>Payments recorded</small><b>{money(data.payments.paidCents)}</b></span><span><small>Outstanding</small><b>{money(data.payments.outstandingCents)}</b></span></div>{data.payments.items.length > 0 && <div className="portal-payment-list">{data.payments.items.slice(0, 8).map((payment) => <span key={payment.id}><i className={payment.status === "Paid" ? "paid" : ""}>{payment.status}</i><b>{payment.description}</b><small>{shortDate(payment.paidDate || payment.dueDate)}</small><strong>{money(payment.amountCents)}</strong></span>)}</div>}</section>
      </div>
    </div>
  </main>;
}
