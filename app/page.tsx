...
function BreedingView({ data, openCreate, openEdit, openDocumentUpload, remove }: ViewProps) {
  ...
  const openDogProfile = (dog: Dog) => {
    setSelectedDogId(dog.id);
    const profile = window.open(`/dogs/${dog.id}`, "_blank", "noopener,noreferrer");
    if (profile) profile.opener = null;
  };
  return <div className="grid two-one">
    <Section eyebrow="Dog Matrix" title="Breeding dogs" action={<button className="ghost" onClick={() => openCreate("dogs")}>Add dog</button>}>
      {data.dogs.length ? <div className="card-grid">{data.dogs.map((dog) => <article key={dog.id} className={selectedDog?.id === dog.id ? "record-card selected" : "record-card"} role="link" tabIndex={0} aria-label={`Open ${dog.name} profile in a new tab`} onClick={() => openDogProfile(dog)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openDogProfile(dog); } }}><span className="avatar">{initials(dog.name)}</span><div><h3>{dog.name}</h3><p>{[dog.role, dog.sex, dog.color].filter(Boolean).join(" / ")}</p><small>Open complete profile</small></div><Status tone={dog.status === "Active" ? "good" : "neutral"}>{dog.status}</Status><footer><a href={`/dogs/${dog.id}`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}><ExternalLink size={14} /> Profile</a><button onClick={(event) => { event.stopPropagation(); openEdit("dogs", dog as unknown as Record<string, unknown>); }}>Edit</button><button onClick={(event) => { event.stopPropagation(); remove("dogs", dog.id, dog.name); }}>Delete</button></footer></article>)}</div> : <Empty title="No breeding dogs" text="Add dams and sires with health, registry, acquisition, and notes." action="Add dog" onAction={() => openCreate("dogs")} />}
    </Section>
