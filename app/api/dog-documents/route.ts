import { deleteDogDocument, uploadDogDocument } from "../../../db/dog-documents";

export async function POST(request: Request) {
  try {
    const document = await uploadDogDocument(await request.formData());
...
export async function DELETE(request: Request) {
  try {
    ...
    await deleteDogDocument(documentId);
