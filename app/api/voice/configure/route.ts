...
export async function POST(request: Request) {
  ...
  if (!accountSid || !authToken || !webhookBase) {
    return Response.json({ error: "Twilio credentials and webhook base URL must be configured before syncing phone lines." }, { status: 503 });
  }

  const managedLines = [
    { id: "main", phone: normalizePhone(process.env.SWVAOS_CALLER_ID || "+18555065425"), friendlyName: "SWVAOS Main Line" },
    { id: "pup-lift", phone: normalizePhone(process.env.SWVAOS_PUP_LIFT_NUMBER || "+17158889526"), friendlyName: "Pup-Lift Support" },
  ];
  ...
  try {
    ...
    const results = await Promise.all(managedLines.map(async (line) => {
      ...
      const updated = await client.incomingPhoneNumbers(record.sid).update({ friendlyName: line.friendlyName, voiceUrl, voiceMethod: "POST" });
    ...
    return Response.json({ configured, voiceUrl, lines: results }, { status: configured ? 200 : 207, headers: { "cache-control": "no-store" } });
  } catch (error) {
