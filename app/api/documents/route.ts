import { deleteBuyerDocument, uploadBuyerDocument } from "../../../db/documents";

export async function POST(request: Request) {
  try {
    const document = await uploadBuyerDocument(await request.formData());
...
export async function DELETE(request: Request) {
  try {
    ...
    await deleteBuyerDocument(documentId);
