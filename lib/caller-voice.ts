...
function publicMenu(response: InstanceType<typeof twilio.twiml.VoiceResponse>, profile: CallerCrmProfile, calledNumber: string | null | undefined) {
  ...
  if (profile.recognized) {
    ...
    gather.say(voice, `Thank you for calling Southwest Virginia Chihuahua. Hello ${firstName}. We found a possible family account connected to this phone number. Press 1 to verify your account and hear private puppy, application, and payment information. Press 2 for current puppy availability and the application process. Press 3 for pickup, delivery, and transportation information. Press 4 for general payment and financing information. Press 5 for Pup-Lift information. Press 6 to leave a message. Press 7 to speak with our team. Press 9 to repeat this menu.`);
  } else {
    gather.say(voice, "Thank you for calling Southwest Virginia Chihuahua. Press 1 if you are an existing applicant or buyer and need account help. Press 2 for current puppy availability and the application process. Press 3 for pickup, delivery, and transportation information. Press 4 for general payment and financing information. Press 5 for Pup-Lift information. Press 6 to leave a message. Press 7 to speak with our team. Press 9 to repeat this menu.");
  }
}
...
export function verificationPromptVoiceResponse(profile: CallerCrmProfile, calledNumber: string | null | undefined, attempt = 1) {
  ...
  gather.say(voice, `${attempt > 1 ? "That ZIP code did not match. " : ""}For privacy, enter the five digit ZIP code on your family account now.`);
...
export function verificationFailedVoiceResponse(calledNumber: string | null | undefined, attempt: number) {
  const response = new twilio.twiml.VoiceResponse();
  if (attempt >= 3) {
    response.say(voice, "We could not verify the account after three attempts. No private information was provided. Please leave a message or speak with our team for help updating your account information.");
    response.redirect(routeWithContext(incomingPath, calledNumber));
  } else {
...
function secureMenu(response: InstanceType<typeof twilio.twiml.VoiceResponse>, profile: CallerCrmProfile, calledNumber: string | null | undefined, session: string) {
  ...
  gather.say(voice, `Welcome to your private family account, ${firstName}. Press 1 for your assigned puppy and the newest development update. Press 2 for application status and current next steps. Press 3 for payments, balance, and payment plan information. Press 4 for pickup or delivery information. Press 5 to hear the newest puppy update again. Press 6 to leave a message. Press 7 to speak with our team. Press 8 to return to the main menu. Press 9 to repeat this private menu.`);
}
...
export function accountVoiceResponse(profile: CallerCrmProfile, digit: string, calledNumber: string | null | undefined, session: string) {
  ...
  else if (digit === "4") response.say(voice, profile.puppies.length ? `Pickup or delivery will be connected to ${profile.puppies.map((puppy) => puppy.name).join(" and ")}. A requested date is not confirmed until our team approves it and all required account items are complete.` : "There is no assigned puppy on this account yet. Pickup or delivery is arranged only after a puppy is assigned and account requirements are complete.");
...
export function menuVoiceResponse(profile: CallerCrmProfile, digit: string, calledNumber?: string | null) {
  ...
  else if (digit === "3") response.say(voice, "Ground transportation is arranged individually after a puppy is assigned. Requested dates and meeting locations are not confirmed until approved by our team. Buyers can submit pickup or delivery requests through the Puppy Portal.");
  else if (digit === "4") response.say(voice, "Payment plans may be available for eligible standard-size puppies when approved in advance. Financing terms, required deposits, due dates, and payment methods are documented in the buyer's account and agreements. Micro or unusually tiny puppy care charges are not financed.");
