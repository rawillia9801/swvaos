...
export async function renderPaymentAgreementPdf(input: PaymentAgreementInput) {
  ...
  const paragraph = (text: string, size = 9.3) => {
    const lines = wrap(regular, text, size, width - margin * 2);
    ensure(lines.length * 13 + 8);
    for (const line of lines) { page.drawText(line, { x: margin, y, size, font: regular, color: ink }); y -= 13; }
    y -= 6;
  };
  ...
  const totalPayments = amountFinanced + input.financeChargeCents + input.monthlyAdminFeeCents * input.installmentCount;
  ...
  y -= 22;
  paragraph("Complete financial terms must be entered before either party signs. This agreement supplements the Deposit & Reservation Agreement and the final Bill of Sale & Health Guarantee.");
  ...
  row("Finance charge", money(input.financeChargeCents));
  row("Amount financed", money(amountFinanced));
  ...
  paragraph("The figures in this disclosure and the payment schedule must agree. Any mathematical inconsistency must be corrected in writing so the schedule reflects the lawful agreed terms and payments already made.");

  heading("5. Fees and Credits");
  row("Monthly administrative fee", money(input.monthlyAdminFeeCents));
  ...
  paragraph("No fee will be charged in an amount or manner prohibited by applicable law. An on-time credit applies only when the complete scheduled payment clears by its due date and may not reduce the required final payment below zero.");

  heading("6. Payment Schedule");
  for (let index = 1; index <= input.installmentCount; index += 1) {
    row(`Installment ${index}`, `${money(input.installmentAmountCents)} — ${input.frequency}`);
  }
  ...
  const clauses = [
    ...
    ["10. Late and Returned Payments", "A payment is late when the full scheduled amount has not cleared by the end of the stated grace period. Returned, reversed, disputed, stopped, or rejected payments remain unpaid and must be replaced promptly."],
  ...
  paragraph("I understand that this is a binding payment obligation. I reviewed the completed financial terms and payment schedule and agree to pay the total obligation when due.", 10);
  y -= 8;
